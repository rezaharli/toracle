...
func (c *LB5Controller) GetLatestData() ([]toolkit.M, error) {
	sqlQuery := "SELECT DISTINCT TAHUN,BULAN,TAHUN||''||BULAN AS TAHUNBULAN FROM F_FA_RASIO_KEUANGAN ORDER BY TAHUN DESC,BULAN DESC"
...
func (c *LB5Controller) DeleteSelectedData(year int, month int) error {
	conn := helpers.Database()
	var res interface{}
	res, err := conn.Execute(
		dbflex.
			From("F_FA_RASIO_KEUANGAN").
...
func (c *LB5Controller) ExecProcess(year string, month string) error {
