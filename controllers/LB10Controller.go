...
type LB10Controller struct {
	*Base
}

func NewLB10Controller() *LB10Controller {
	return new(LB10Controller)
}

func (c *LB10Controller) ReadAPI() error {
	log.Println("\n--------------------------------------\nReading LB10 API")
	var err error
...
func (c *LB10Controller) GetAPIDatas(payload []byte, month string, year string) ([]toolkit.M, error) {
	log.Println("Get LB10")
	config := clit.Config("lb", "lb10", map[string]interface{}{}).(map[string]interface{})
...
func (c *LB10Controller) InsertAPIDatas(results []toolkit.M, jsonconf string) error {
	...
	for _, result := range results {
		...
		param := helpers.InsertParam{
			TableName: "F_FA_IKHTISAR_PENDAPATAN",
			Data:      rowData,
		}
...
func (c *LB10Controller) SetParamBody(year string, month string) []byte {
...
func (c *LB10Controller) GetLatestData() ([]toolkit.M, error) {
	sqlQuery := "SELECT DISTINCT TAHUN,BULAN,TAHUN||''||BULAN AS TAHUNBULAN FROM F_FA_IKHTISAR_PENDAPATAN ORDER BY TAHUN DESC,BULAN DESC"
...
func (c *LB10Controller) DeleteSelectedData(year int, month int) error {
	conn := helpers.Database()
	var res interface{}
	res, err := conn.Execute(
		dbflex.
			From("F_FA_IKHTISAR_PENDAPATAN").
...
func (c *LB10Controller) ExecProcess(year string, month string) error {
	...
	err = c.InsertAPIDatas(results, "lb10")
	if err != nil {
		log.Println(err.Error())
		return err
	}
