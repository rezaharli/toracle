...
func (c *LB13Controller) GetLatestData() ([]toolkit.M, error) {
	sqlQuery := "SELECT DISTINCT TAHUN,BULAN,TAHUN||''||BULAN AS TAHUNBULAN FROM F_FA_REALISASI_PUSAT_PERJENIS ORDER BY TAHUN DESC,BULAN DESC"
...
func (c *LB13Controller) DeleteSelectedData(year int, month int) error {
	conn := helpers.Database()
	var res interface{}
	res, err := conn.Execute(
		dbflex.
			From("F_FA_REALISASI_PUSAT_PERJENIS").
...
func (c *LB13Controller) ExecProcess(year string, month string) error {
