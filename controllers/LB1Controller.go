...
type LB1Controller struct {
	*Base
}

func NewLB1Controller() *LB1Controller {
	return new(LB1Controller)
}

func (c *LB1Controller) ReadAPI() error {
	log.Println("\n--------------------------------------\nReading LB1 API")
	var err error
...
func (c *LB1Controller) GetAPIDatas(payload []byte, month string, year string) ([]toolkit.M, error) {
	log.Println("Get LB1")
	config := clit.Config("lb", "lb1", map[string]interface{}{}).(map[string]interface{})
...
func (c *LB1Controller) InsertAPIDatas(results []toolkit.M, jsonconf string) error {
...
func (c *LB1Controller) SetParamBody(year string, month string) []byte {
...
func (c *LB1Controller) GetLatestData() ([]toolkit.M, error) {
	sqlQuery := "SELECT DISTINCT TAHUN,BULAN,TAHUN||''||BULAN AS TAHUNBULAN FROM F_FA_NERACA_KOMP ORDER BY TAHUN DESC,BULAN DESC"
...
func (c *LB1Controller) DeleteSelectedData(year int, month int) error {
	conn := helpers.Database()
	var res interface{}
	res, err := conn.Execute(
		dbflex.
			From("F_FA_NERACA_KOMP").
...
func (c *LB1Controller) ExecProcess(year string, month string) error {
	...
	err = c.InsertAPIDatas(results, "lb1")
	if err != nil {
		log.Println(err.Error())
		return err
	}
