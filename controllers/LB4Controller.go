...
type LB4Controller struct {
	*Base
}

func NewLB4Controller() *LB4Controller {
	return new(LB4Controller)
}

func (c *LB4Controller) ReadAPI() error {
	log.Println("\n--------------------------------------\nReading LB4 API")
	var err error
...
func (c *LB4Controller) GetAPIDatas(payload []byte, month string, year string) ([]toolkit.M, error) {
	log.Println("Get LB4")
	config := clit.Config("lb", "lb4", map[string]interface{}{}).(map[string]interface{})
...
func (c *LB4Controller) InsertAPIDatas(results []toolkit.M, jsonconf string) error {
...
func (c *LB4Controller) SetParamBody(year string, month string) []byte {
...
func (c *LB4Controller) GetLatestData() ([]toolkit.M, error) {
	sqlQuery := "SELECT DISTINCT TAHUN,BULAN,TAHUN||''||BULAN AS TAHUNBULAN FROM F_FA_ARUS_KAS ORDER BY TAHUN DESC,BULAN DESC"
...
func (c *LB4Controller) DeleteSelectedData(year int, month int) error {
	conn := helpers.Database()
	var res interface{}
...
func (c *LB4Controller) ExecProcess(year string, month string) error {
	...
	err = c.InsertAPIDatas(results, "lb4")
	if err != nil {
		log.Println(err.Error())
		return err
	}
